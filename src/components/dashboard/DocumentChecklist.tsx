...
export const DocumentChecklist = ({ userId, className }: DocumentChecklistProps) => {
  ...
  const documents = requiredDocuments.map(doc => ({
    ...doc,
    uploaded: uploadedDocs.some(uploaded => 
      uploaded.includes(doc.id.replace('-', ' ')) || 
      doc.id.includes(uploaded.replace(' ', '-'))
    )
  }));
                    ...
                    {docs.map((doc) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className={cn(
                            "text-sm font-medium",
                            doc.uploaded && "text-green-700 dark:text-green-400"
                          )}>
                            {doc.name}
